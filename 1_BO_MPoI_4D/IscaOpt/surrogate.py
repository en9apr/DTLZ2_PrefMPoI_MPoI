...
class Surrogate(object):
    ...
    def __init__(self, init_X, init_Y, kernel, lb, ub, i, restarts=10, verbose=True): # APR added lb, ub and verbose =True
        ...
        self.lb = lb #APR
        self.ub = ub #APR
        self.Xtr = (self.X-self.lb)/(self.ub - self.lb) #APR
        self.ybar = np.mean(self.Y)
        self.ystd = np.std(self.Y)
        
        #self.objective_number = i # APR added
    ...
    def train(self, fix_min=1e-4, fix_max=1e8, optimizer='lbfgs', \
        ...
        #APR
#        if(self.objective_number == 0):
#            Xtrain = self.Xtr[:,2:]
#            print("train(): objective_number: ", self.objective_number)    #APR
#            print("train(): Xtr.shape: ", self.Xtr.shape)    #APR
#            print("train(): Xtrain.shape: ", Xtrain.shape)    #APR
#            
#        else:
#            Xtrain = self.Xtr[:,0:2]
#            print("train(): objective_number: ", self.objective_number)    #APR
...
#            print("train(): Xtrain.shape: ", Xtrain.shape)    #APR            
        ...
        min_noise = 1e-4
        fix_min_variance = 1e-4
        fix_max_variance = 1e8
        dists = distance.pdist(self.Xtr, metric='euclidean') # APR
        fix_min_lengthscale = max(1e-4,min(dists))
        fix_max_lengthscale = np.sqrt(4)
        model[pnames[0]].constrain_bounded(fix_min_variance, fix_max_variance)
        model[pnames[1]].constrain_bounded(fix_min_lengthscale, fix_max_lengthscale)
        model[pnames[-1]].constrain_fixed(min_noise)
        ...
        if np.all(check_nan):
            ...
            model = GP.models.GPRegression(self.Xtr, self.Ytr, self.kernel) # APR
            model.constrain_positive('')
            ...
            if np.all(check_nan == True):
                self.save_data('nan_debug', self.Xtr, self.Ytr) # APR
        ...
        print("train(): model: ", model)    #APR
        lengthscales = np.stack((model.kern.lengthscale[0], model.kern.lengthscale[1], model.kern.lengthscale[2], model.kern.lengthscale[3]), axis=-1) #APR
        print("train(): lengthscales: ", lengthscales) #APR
    ...
    def predict(self, x):
        ...
        """
        Xtest = (x-self.lb)/(self.ub - self.lb)
...
#            Xpredict = Xtest[:,2:]
##            print("train(): objective_number: ", self.objective_number)    #APR
##            print("train(): Xtest.shape: ", Xtest.shape)    #APR
##            print("train(): Xpredict.shape: ", Xpredict.shape)    #APR             
#        else:
#            Xpredict = Xtest[:,0:2]
##            print("train(): objective_number: ", self.objective_number)    #APR
##            print("train(): Xtest.shape: ", Xtest.shape)    #APR
...
class MultiSurrogates(object):
    ...
    def __init__(self, xtr, ytr, kernels, lb, ub, verbose=False):
        '''
        ...
        # APR added surrogate
        #####################
        self.surrogate1 = self.models[0]
        self.surrogate2 = self.models[1]
		#####################
        # APR added surrogate
        #####################
    ...
    def train_models(self):
        ...
        '''
        ####################################
        # APR added i to Surrogate arguments
        ####################################
        models = []
        for i in range(self.n_models):
            models.append(Surrogate(self.xtr, np.reshape(self.ytr[:,i], \
                                    (-1, 1)), self.kernels[i], self.lb, self.ub, i, verbose=self.verbose))
        return models
    ...
    def predict(self, x):
        ...
        return np.concatenate((mean),axis=1), np.concatenate((var),axis=1) 
