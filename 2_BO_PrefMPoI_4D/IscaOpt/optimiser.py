...
# Send centroid as class to CMA-ES
class X0:
    def __init__(self, dimensions, lowerb, upperb):
        self.lowerb = lowerb
        self.upperb = upperb
        self.dimensions = dimensions
    def __call__(self):
        return ((np.random.random(self.dimensions) * np.array(list(self.upperb - self.lowerb)))+ np.array(list(self.lowerb)))
...
class Optimiser(object):
    ...
    def EMO(func, fargs=(), fkwargs={}, cfunc=None, cargs=(), ckwargs={},\
        ...
        while True:
            ...
            cma_es_upper = (ub - lb) / (ub - lb)
            cma_es_lower = (lb - lb) / (ub - lb)
        
            print("cma_es_upper: ", cma_es_upper)
            print("cma_es_lower: ", cma_es_lower)
            
            start = time.time()
            if n_dim > 1:
                xopt = Optimiser.CMA_ES(toolbox, X0(n_dim, cma_es_lower, cma_es_upper), cma_sigma, \
